...
public class OrderServiceImpl implements OrderService {
    ...
    public OrderSubmitVO submitOrder(OrdersSubmitDTO ordersSubmitDTO) {
        ...
        for (Map.Entry<Object, Object> entry : cartEntries.entrySet()) {
            ...
            ShoppingCart cartItem = JSON.parseObject(json, ShoppingCart.class);
        ...
        log.info("向订单表明细表插入n条数据，订单号：{}", orders.getNumber());

        // 清空当前用户的购物车数据
        try {
            stringRedisTemplate.delete(cartKey); // 直接删除整个key更干净
            log.info("清空购物车成功");
        } catch (Exception e) {
            log.error("清空购物车失败", e);
            // 可发送补偿消息
...
*/
    ...
    public void repetition(Long id) {
        ...
        for (ShoppingCart cartItem : shoppingCartList) {
            stringRedisTemplate.opsForHash().put(cartKey, cartItem.getDishId() != null ? 
                String.valueOf(cartItem.getDishId()) : String.valueOf(cartItem.getSetmealId()), 
